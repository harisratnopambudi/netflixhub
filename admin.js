...
async function handleSubmit(event) {
    ...
    try {
        ...
        const params = new URLSearchParams({
            ...
            // Force text for PINs starting with 0
            pin: (data.pin && data.pin.startsWith('0')) ? "'" + data.pin : (data.pin || ''),
...
function getWhatsAppMessage(member) {
    ...
    if (needsReminder) {
        ...
        // Handle PIN display (remove leading quote if present for display)
        let displayPin = member.pin || '-';
        if (displayPin.startsWith("'")) {
            displayPin = displayPin.substring(1);
        }

        // Format date with full month name for the message
        const dateObj = new Date(member.dueDate);
        ...
        return encodeURIComponent(
            ...
            `PIN Profil: ${displayPin}\n` +
            `Akun aktif hingga: ${dateStr}\n\n` +
